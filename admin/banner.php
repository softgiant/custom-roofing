...
/*===========  INSERT QUERY START ===========*/
	...
	{	
		$table = BANNER;
			...
			$upload_dir='../images/banner_gallery/';
...
/*===========  INSERT QUERY END ===========*/
	...
	{
		$SlideVal=$obj->runSql("SELECT * FROM ".BANNER." WHERE id = ".$_GET['LinkID']);
	}
...
/*=========== STATUS CHANGE START ================*/
	if(isset($_GET['mode']) && ($_GET['mode'] == 'Y' || $_GET['mode'] == 'N'))
	{
		...
		$obj->AddEditQuery($table,$fieldsValue,$WhereClause);
		header("location:banner.php?page=".$_GET['page']);
	}
/*=========== STATUS CHANGE END ================*/
...
/*=========== DELETE START ================*/
	...
	{
		$del_content=$obj->runSql("SELECT `dfile` FROM ".BANNER." WHERE id = ".$_GET['id']);
		$table = BANNER;
		$WhereClause = "`id` = ".$_GET['id'];
		$obj->deleteQuery($table,$WhereClause);
		if($del_content['dfile'] != '') unlink('../images/banner_gallery/'.$del_content['dfile']);		
		header("location:banner.php?page=".$_GET['page']);		
	}
/*=========== DELETE END ================*/	
?>
...
<!--
	...
	{
		document.afrm.action='banner.php?Show='+id;
	...
	function del(uid,page)
	{
		...
		{
		   document.afrm.action='banner.php?mode=del&id='+uid+'&page='+page;
		   document.afrm.submit();
		}
	}
//-->
		  ...
		  <article class="col-sm-12 col-md-12 col-lg-6">
			<div class="jarviswidget" id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false">
			  <header> <span class="widget-icon"> <i class="fa fa-edit"></i> </span>
				<h2><?PHP echo ($_GET['LinkID']!='' ? 'Edit' : 'Add'); ?> Banner Name</h2>
			  </header>
			  <div>
				  ...
				  <form id="smart-form-register" class="smart-form" name="frm" method="POST" action="" enctype="multipart/form-data">
					...
					<fieldset>
                      ...
                      <label class="textarea">
                        ...
                        <section>
                          <label class="label">Banner File</label>
        ...
        <form name="afrm" method="post" action="" enctype="multipart/form-data">
          <div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" data-widget-editbutton="false">
			...
			<?PHP
                ...
                $selectQuery = "SELECT * FROM ".BANNER." ".$where_clause." ORDER BY `id` ASC";
			...
			?>
                    <tr>
                      ...
                      <td><?PHP echo $SlideVal['description']; ?></td>
                      ...
                      <td>				  
						<?php
							...
							{
						?>
								<a href = "<?PHP echo ADMIN_SITE_URL; ?>banner.php?staID=<?php echo($SlideVal['id']);?>&mode=N&page=<?PHP echo($_GET['page']!=''?$_GET['page']:'1'); ?>" title = "Click Here To Inactive"><i class="fa fa-active"></i></a>
						<?php
							}
							else
							{
						?>
								<a href = "<?PHP echo ADMIN_SITE_URL; ?>banner.php?staID=<?php echo($SlideVal['id']);?>&mode=Y&page=<?PHP echo($_GET['page']!=''?$_GET['page']:'1'); ?>" title = "Click Here To Active"><i class="fa fa-inactive"></i></a>
						<?php
							}	
						?>
                        &nbsp;&nbsp;
                        <a href="<?PHP echo ADMIN_SITE_URL; ?>banner.php?LinkID=<?php echo $SlideVal['id'];?>&page=<?PHP echo($_GET['page']!=''?$_GET['page']:'1'); ?>" title="Click Here To Edit"><i class="fa fa-pencil"></i></a>
                        &nbsp;&nbsp;
