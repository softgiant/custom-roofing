...
/*=========== STATUS CHANGE START ================*/
	if(isset($_GET['mode']) && ($_GET['mode'] == 'Y' || $_GET['mode'] == 'N'))
	{
		...
		header("location:testimonial.php?page=".$_GET['page']);
	}
/*=========== STATUS CHANGE END ================*/
...
/*=========== DELETE START ================*/
	...
	{
		$del_content=$obj->runSql("SELECT * FROM ".TESTIMONIAL." WHERE id = ".$_GET['id']);
		$table = TESTIMONIAL;
		$WhereClause = "`id` = ".$_GET['id'];
		$obj->deleteQuery($table,$WhereClause);
		
		$explodefile = explode(",", $del_content['dfile']);
		for($n=0; $n<=4; $n++){
			if($explodefile[$n] != '') unlink('../images/blog_gallery/'.$explodefile[$n]);
		}
...
<!--
	function show_you(id)
	{
		document.frm.action='testimonial.php?Show='+id;
		document.frm.submit();
	}
	function del(uid,page)
	{
		...
		{
		   document.frm.action='testimonial.php?mode=del&id='+uid+'&page='+page;
		   document.frm.submit();
		}
	}
//-->
    ...
    <section id="widget-grid" class="">
      <div class="row">
		...
		<?PHP
			...
			$selectQuery = "SELECT * FROM ".TESTIMONIAL." ".$where_clause." ORDER BY `id` ASC";
		...
		?>
                  <thead>
                    <tr>
                      ...
                      <th data-class="expand"><i class="fa fa-fw text-muted hidden-md hidden-sm hidden-xs"></i>Testimonial Title</th> 
                     <!--  <th data-hide="phone,tablet">Order No</th> -->
			...
			?>
                    <tr>
                     <!--  <td><?PHP //echo $i; ?></td> -->					  <td><?PHP echo $SlideVal['order_id']; ?></td>
                      ...
                      <td>				  
						<?php
							...
							{
						?>
								<a href = "<?PHP echo ADMIN_SITE_URL; ?>testimonial.php?staID=<?php echo($SlideVal['id']);?>&mode=N&page=<?PHP echo($_GET['page']!=''?$_GET['page']:'1'); ?>" title = "Click Here To Inactive"><i class="fa fa-active"></i></a>
						<?php
							}
							else
							{
						?>
								<a href = "<?PHP echo ADMIN_SITE_URL; ?>testimonial.php?staID=<?php echo($SlideVal['id']);?>&mode=Y&page=<?PHP echo($_GET['page']!=''?$_GET['page']:'1'); ?>" title = "Click Here To Active"><i class="fa fa-inactive"></i></a>
						<?php
							}	
						?>&nbsp;<a href="<?PHP echo ADMIN_SITE_URL; ?>edit_testimonial.php?PageID=<?php echo $SlideVal['id'];?>&page=<?PHP echo($_GET['page']!=''?$_GET['page']:'1'); ?>" title="Click Here To Edit"><i class="fa fa-pencil"></i></a>
